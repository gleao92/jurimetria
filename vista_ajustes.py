...
def render(ctx):
    ...
    st.markdown('<span class="passo">4 · Captura Local (PC do Escritório)</span>', unsafe_allow_html=True)
    st.info("Como os tribunais exigem IP do Brasil, a captura automática é executada no seu computador local usando o script `capturar.bat` e enviada ao banco de dados na nuvem.")
    conectada = st.toggle("Ativar monitoramento de captura local",
                          value=cfg.get("conectada", True))
    if conectada != cfg.get("conectada", True):
